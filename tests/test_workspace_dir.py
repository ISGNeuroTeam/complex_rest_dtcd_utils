...
from dtcd_server.utils.filesystem_workspacemanager import FilesystemWorkspaceManager
from dtcd_server.utils.workspacemanager_exception import WorkspaceManagerException
...
class TestWorkspaceDir(TransactionTestCase):
    ...
    def test_workspace_invalid_path_short(self):
        try:
            dirs = self.fsw_manager.read_all_dir(workspace_path='qwerty')
        except WorkspaceManagerException as e:
            self.assertTrue(True)
            return
    ...
    def test_workspace_invalid_path_long(self):
        try:
            dirs = self.fsw_manager.read_all_dir(workspace_path='qwerty/booby/trap')
        except WorkspaceManagerException as e:
            self.assertTrue(True)
            return
        self.assertTrue(False, 'invalid path exception not thrown')
    ...
    def test_post_dir_with_no_name(self):
        try:
            self.fsw_manager.write_dir({'age': 18}, workspace_path='')
        except WorkspaceManagerException as e:
            self.assertTrue(True)
            return
    ...
    def test_post_dir_empty_name(self):
        try:
            self.fsw_manager.write_dir({'name': ''}, workspace_path='')
        except WorkspaceManagerException as e:
            self.assertTrue(True)
            return
        self.assertTrue(False)
    ...
    def test_post_dir_name_with_slashes(self):
        try:
            self.fsw_manager.write_dir({'name': 'booby/trap'}, workspace_path='')
        except WorkspaceManagerException as e:
            self.assertTrue(True)
            return
        self.assertTrue(False)
